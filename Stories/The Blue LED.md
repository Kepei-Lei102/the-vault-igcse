...
- **The screens in your pocket:** every white-backlit LCD — which **Input and Output Devices** *(not yet released)* taught you is really an "LED screen" — and every true LED/OLED-era display descends from this act; so do Blu-ray's 405 nm pens, LED streetlights, and the solar lantern replacing kerosene across the off-grid world.
...
- **Input and Output Devices** *(not yet released)* — the "LED screen" misconception (LED-backlit LCD vs OLED): the backlight that makes the modern screen possible is this story's white light.
- **Secondary Storage** *(not yet released)* — the CD→DVD→Blu-ray wavelength ladder (780→650→**405 nm**): the ladder's final rung is Nakamura's violet laser.
