...
($n$ = carrier density, $t$ = slab thickness along $\mathbf{B}$). Two gifts follow. Practically, $V_H \propto B$ makes the slab a **magnetometer** — the Hall probe is how a lab (and the magnetic-field sensor row of **Sensors and Control Systems** *(not yet released)*, and the lid-closed sensor in a laptop) measures fields. Deeply, the *sign* of $V_H$ reveals the *sign of the carriers*: metals answer "negative, as expected" — and some semiconductors answer **positive**, forcing physics to take seriously the idea that missing electrons ("holes") flow as real positive carriers. A tabletop voltage, quietly reporting quantum mechanics. (Note why $V_H$ is tiny in copper but healthy in a semiconductor: $n$ sits in the denominator — fewer carriers must each drift *faster* to carry the same $I$, and faster drift means a bigger sideways shove.)
...
- **CS bridge:** **Sensors and Control Systems** *(not yet released)* — the magnetic-field row's Hall probe and reed switch are this card's physics in a sensor's costume; a laptop's lid-closed detection is a Hall slab reading a magnet.
