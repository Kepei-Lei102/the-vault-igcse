...
Union distributes over intersection, and intersection distributes over union. This mirrors the Boolean algebra equivalents ($\text{OR}$ distributes over $\text{AND}$, and vice versa) — see **Boolean Algebra** *(not yet released)*.
...
These are essential for Venn diagram shading problems and connect directly to **Boolean Algebra** *(not yet released)*:
...
- **Logic:** **Boolean Algebra** *(not yet released)* — $\cup \leftrightarrow$ OR, $\cap \leftrightarrow$ AND, $' \leftrightarrow$ NOT
